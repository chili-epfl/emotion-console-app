...
#        rate.sleep()
    ...
    for i in range(len(leterList)):
        distances = buildMessage(distDynEigen[0][leterList[i]],leterList[i])
...
"""
def buildMessage(dataMsg, shape):
    
    distances = Path()
    distances.header.frame_id = shape
        
    for j in range(len(dataMsg)):
        point = PoseStamped()
        point.pose.position.x = j+1
        point.pose.position.y = dataMsg[j]
        distances.poses.append(deepcopy(point))
...
def calcDistance(projRefShapes, projDemoShapes, leterList):
    ...
    for leter in projRefShapes:
        ...
        for i in range(len(projDemoShapes[leter])):
            ...
            distances[leter].append(distRound)        
